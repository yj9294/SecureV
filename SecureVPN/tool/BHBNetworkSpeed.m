...
- (void)startMonitoringNetworkSpeed{
    ...
    _timer = [NSTimer scheduledTimerWithTimeInterval:self.intervalTime target:self selector:@selector(netSpeedNotification) userInfo:nil repeats:YES];
}
...
{
    bytes = bytes / self.intervalTime;
    if(bytes < 1024)     // B
    {
        return [NSString stringWithFormat:@"%dBps", bytes];
    }
    else if(bytes >= 1024 && bytes < 1024 * 1024) // KB
    {
        return [NSString stringWithFormat:@"%dKbps", bytes / 1024];
    }
    else if(bytes >= 1024 * 1024 && bytes < 1024 * 1024 * 1024)   // MB
    {
        return [NSString stringWithFormat:@"%.1fMbps", (double)bytes / (1024 * 1024)];
    }
    else    // GB
    {
        return [NSString stringWithFormat:@"%.1fGbps", (double)bytes / (1024 * 1024 * 1024)];
    }
}
